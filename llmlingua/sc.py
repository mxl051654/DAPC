...
class SelectiveContext:
    ...
    def _prepare_model(self):
        ...
        # Load tokenizer
        if self.lang == 'zh':
            self.tokenizer = BertTokenizer.from_pretrained(f'{hf_dir}/uer/gpt2-chinese-cluecorpussmall')
        elif self.lang == 'en':
            self.tokenizer = GPT2Tokenizer.from_pretrained(f'{hf_dir}/openai-community/gpt2')
        else:
            raise NotImplementedError()

        if self.model_type == 'gpt2':
            if self.lang == 'zh':
                self.model = GPT2LMHeadModel.from_pretrained(f'{hf_dir}/uer/gpt2-chinese-cluecorpussmall')
            else:
                self.model = GPT2LMHeadModel.from_pretrained(f'{hf_dir}/openai-community/gpt2')
            ...
            self.tokenizer = LlamaTokenizer.from_pretrained(f'{hf_dir}/meta-llama/Llama-2-7b-chat-hf', token='LLaMA TOKEN')
            print("Before model")
            config = AutoConfig.from_pretrained(f'{hf_dir}/meta-llama/Llama-2-7b-chat-hf', token='LLaMA TOKEN')
            print("After config")
            self.model = LlamaForCausalLM.from_pretrained(f'{hf_dir}/meta-llama/Llama-2-7b-chat-hf', config=config,
                                                          token='LLaMA TOKEN')
            print("Before DEVICE")
            self.model.to(DEVICE)
            print("Before eval")
    ...
    def __call__(self, text: str, reduce_ratio: float = 0.35, reduce_level: str = 'phrase') -> List[str]:
        ...
        sents = re.split(self.sent_tokenize_pattern, context)  # r"(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)\s"
        sents = [sent.strip() for sent in sents if sent.strip()]

        # You want the reduce happen at sentence level, phrase level, or token level?
        assert reduce_level in ['sent', 'phrase',
                                'token'], f"reduce_level should be one of ['sent', 'phrase', 'token'], got {reduce_level}"
        sent_lus, phrase_lus, token_lus = self._lexical_unit(sents)  # NOTE 计算信息量
        # print(phrase_lus, '^^^^')
        lexical_level = {
            'sent': sent_lus,
            'phrase': phrase_lus,
        ...
        context, masked_sents = self.self_info_mask(lexical_level[reduce_level].text,
                                                    lexical_level[reduce_level].self_info, reduce_level)
