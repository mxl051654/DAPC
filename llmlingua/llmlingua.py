...
class PromptCompressor:
    ...
    def compress_prompt(
        ...
        origin_tokens = len(
            self.tokenizer("\n\n".join([instruction] + context + [question]).strip()).input_ids
        )
        ...
        compressed_tokens = len(self.tokenizer(compressed_prompt).input_ids)
            ...
            for batch in dataloader:
                ...
                for j in range(ids.shape[0]):
                    ...
                    for word, word_prob in zip(words, word_probs):
                        ...
                        num_token = len(self.tokenizer(word).input_ids)
