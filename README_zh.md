...
- torch
- transformers（推荐版本约 4.38.2）
- datasets
- numpy, pandas
- matplotlib, seaborn
- tqdm
- fuzzywuzzy 等用于文本相似度计算的工具
- 本仓库自带的 `longbench/llmlingua` 作为本地 LLMLingua 实现
...
请确保：
- LongBench 数据集放在类似 `data_dir/THUDM/LongBench` 的路径下
...
`compress.py` 中额外提供 `synthetic_compress` 函数，用于将不同方法的 token 重要性分数按权重线性融合，得到“合成”压缩器。例如：
